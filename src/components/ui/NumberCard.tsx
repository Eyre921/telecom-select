    ...
    return (
        <div className={cardClasses} onClick={() => isAvailable && onClick(number)}>
            ...
            {(number.isPremium && isAvailable) && (
                <div className="absolute top-0 right-0 px-2 py-1 text-xs font-bold text-white bg-red-500 rounded-bl-lg rounded-tr-lg">
                    {number.premiumReason || '靓号'}
                </div>
            )}
            {!isAvailable && statusText && (
                <div className="absolute top-0 right-0 px-2 py-1 text-xs font-bold text-white bg-gray-500 rounded-bl-lg rounded-tr-lg">
                    {statusText}
                </div>
            )}
